...
function Skills() {
  const skill = [
    ...
    { text: "Typescript", logo: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" },
    ...
    { text: "HTML", logo: "https://upload.wikimedia.org/wikipedia/commons/5/5a/Data_structure.jpg" },
    { text: "CSS", logo: "https://upload.wikimedia.org/wikipedia/commons/5/5a/Data_structure.jpg" },
  ];
  ...
  return (
    <div className="px-4 ">
      <motion.div
        ...
        initial="initial"
        animate="animate"
        className="text-3xl font-serif font-normal tracking-tight  text-black dark:text-white mb-8"
      >
        Skills
      </motion.div>
      ...
      >
        {skill.map((val, idx) => (
          <motion.div
            key={idx}
            variants={child}
            className="flex items-center  px-2 py-2 bg-neutral-800 dark:bg-neutral-300   text-neutral-200 dark:text-black border border-black dark:border-white rounded-md shadow-sm hover:scale-105 transition-transform duration-200"
          >
          
            <span className="text-sm font-sans tracking-tight font-medium ">{val.text}</span>
          </motion.div>
        ))}
      </motion.div>
    </div>
